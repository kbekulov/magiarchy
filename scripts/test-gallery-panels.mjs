...
export async function testGalleryPanels(page, origin, engine) {
  ...
  for (const width of [320, 390, 820, 1440]) {
    ...
    assert.ok(await page.locator('.scene-panel').evaluateAll(panels => panels.every(panel => {
      ...
      return Math.abs(panel.getBoundingClientRect().width - available) < 2;
    })), 'Each successive panel must use the full artwork width');
    ...
    await page.keyboard.press('Enter');
    await page.waitForURL(url => url.hash === '#panel-7');
    assert.ok(page.url().endsWith('#panel-7'));
    await page.waitForFunction(() => document.querySelector('#panel-jump-links a[aria-current]')?.getAttribute('href') === '#panel-7');
    assert.ok(await page.locator('#panel-7').evaluate(el => el === document.activeElement));
    await page.locator('#panel-7 img').evaluate(img => img.decode());
  ...
  try {
    ...
    await touchPage.waitForURL(url => url.hash === '#panel-7');
    await touchPage.locator('#panel-7 img').evaluate(img => img.decode());
    assert.ok(touchPage.url().endsWith('#panel-7'));
    assert.ok(await touchPage.locator('#panel-7').isVisible());
