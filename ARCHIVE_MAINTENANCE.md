...
Inspect every supplied image. Determine reading order from author direction; ask about unclear sequencing. Multiple compositions for the same beat use unique `composition` labels and appear together under one beat heading, with every image visible. A later composition is not automatically a replacement. In the current river set (r6), sketches 1/2 remain alternative compositions, while sketch 6 continues sketch 5 as a separate successive beat: five beats and six images. Earlier revisions grouped sketches 5/6 as alternatives; that historical grouping must not override the author's revised sequence. Preserve source annotations without treating them as instructions or additional canon. The river panel collection illustrates the existing struggle, surface support, and fish-dressed girl; it does not resolve the Choir's limits, permanent injuries, or larger river mysteries.
