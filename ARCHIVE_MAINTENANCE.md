...
Panel readers use a responsive beat-grouped navigator, not an overflowing thumbnail strip. Each flat card has a numbered beat title and equal-height, uncropped preview bays; alternate compositions remain together with explicit labels. Wrap cards at narrow widths, preserve keyboard/touch links and stable image fragments, and mark the selected destination with `aria-current`. Do not hide a horizontal overflow problem by merely concealing its scrollbar. This navigator is distinct from the full-size reading sequence below it.
