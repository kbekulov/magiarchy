...
A Moment may link its illustration through `artwork: { id, title }`, using the Gallery image ID. The matching Gallery card carries `data-moment` for the reciprocal reader link. Verify both destinations and character metadata. Author approval to adapt an illustration does not establish a named location, Arc, hidden motive, or supernatural effect that the image cannot settle. Only Eyes for You (MOM-025) keeps Lynleit and Felix's church exchange unplaced; its two illustrated lines and setup are approved, while connective prose remains a scene draft. Jealousy and reciprocal attraction are not confirmed by her reprimand.
