...
- Horror/action material: read Dread and Action Direction with current Prose Style and the owning incident or scene. Check isolation, readable geography, sensory evidence, character-specific agency, credible costs, and relief. Use MAGIARCHY's own people, places, and supernatural conditions. Preserve nonphysical and passive Holumns, contemporary institutions, humor, and established abilities. A mood preference cannot resolve an unknown mechanism, add a casualty, or move a reveal. Record proposed mechanics separately until approved. Version affected Docs and preserve previous text; inspect phone and desktop readers, links, version selection, and search after publication.
- Author-directed reference cleanup (21 September 2026): the dread/action preference must stand on its own without game or studio names, borrowed character examples, or external game-research links. This cleanup explicitly includes earlier published document versions, historical Home updates, and generated search entries. Preserve their useful atmosphere and practical guidance, stable version URLs, and all canon boundaries. This narrow exception to historical-text preservation does not authorize rewriting Git history or unrelated historical material.
...
After the Failed Attempt v3 and its two matching Moments follow the September 23 Kyrien origin intake. `docs/kyrien-origin-and-recruitment.md` owns the complete confirmed family history, draft choices, and open later-offence boundary; the Church's Prosecutor General case gives the institutional consequence, and MSF owns the limit on recruitment authority. His father is a military-police commander murdered under suppression sanction. Kyrien pays competent lawyers for years; credible evidence is found before institutional refusal, an interval, and the shop humiliation lead to his decision. He escapes the dinner attack and is detained later without conclusive evidence. Preserve legal representation, paid work and care obligations, no maternal threat, and no unconditional immunity. His first unmistakable magical exposure remains the river. The prosecutor's own magical status and later prohibition are unresolved; later revenge is not redress for the protected original persecution. Earlier Chapter/Moment versions remain historical, with version-scoped advisory notes. No age, date, Arc for the father's death, new faction, or culpability of Fionn/Mikhail is inferred.
...
Keep historical-only artwork last in the Artwork catalog's source order, using the same masonry columns and card sizing as current artwork. Do not force it into a full-width row or apply a separate width cap. Use confirmed creation dates or bounds only; publication dates do not establish creation dates, and undated artwork must not receive invented dates. The earliest cast sketch dates to 2021 or earlier, at least five years before September 2026. Historical author artwork explicitly disconnected from current characters uses empty character metadata, data-historical and data-non-canon set to true, and data-profile-portrait set to false. Keep its historical-only notice prominent in the card and reader, preserve author credit and approximate creation date, and index its dedicated Gallery link. Do not associate the early figures with current characters or reuse this artwork as character or production references.


Kyrien's fixed visual requirements are owned by `docs/character-image-production.md`; `character.js` carries the observable description. Apply them to every medium and future generation brief: anatomical center part, subtly tired eyes, lean light build, lived-in elegance, and a modest 180 cm / 169 cm comparison with Lynleit. The requirements override contrary older depictions without replacing his approved clothing, inventing fatigue causes, or reducing established physical competence. Review current art before deciding a correction is necessary; preserve originals and separately register any corrected derivatives. The September 23 review retains the current front/back reference and two concept sheets: the visible parts follow the turned heads, eyes are restrained, and bodies are lean. The five-character lineup sketch depicts Drake, Sherie, Kyrien, Lynleit and Felix in that order. It stays in Gallery Artwork, outside individual portrait rotation; it does not replace the approved production references. The reference-sheet T-pose is not an everyday stance.

Kyrien's 23 September 2026 design selection supersedes his earlier published portraits, chibis, outfit sheets, and shared scene illustrations. `kyrien-beige-jacket` now owns the sole approved front/back reference pair, separated from the first backlog sheet with rear-view hand corrections. Profile and catalog artwork stays unavailable until new artwork is approved; production T-poses do not enter portrait or chibi pools. Withdrawing illustrations does not withdraw their written scenes. Source and retired files formerly under `backlog/archive/2026-09-23-kyrien-design-intake/` remain recoverable in Git history after the author's 24 September purge.

Before archiving an unused gallery image, check all public and tooling dependencies. The author purged existing backlog/archive files on 20 September and again on 24 September 2026. Deleted sources remain in Git history. Current media and public document history stay active. Normal future backlog archiving remains in force.
...
Music cards preserve the archive's flat artwork and compact metadata layout. Play only the supplied MP3, load audio on visitor action rather than autoplaying or preloading full tracks, and expose separate same-origin MP3 and WAV download links with file sizes. Repeat track is the default, including native looping in the no-JavaScript fallback. The dock repeat button cycles track, playlist, and off. Playlist mode wraps after the last recording; off continues through the remaining tracks and stops at the end. Keep the current mode across internal navigation. Starting another track switches the shared player; filtering and page navigation never pause it. Keep unrecorded concepts inactive. Author-permitted working category assignments are editable catalog metadata, not confirmed soundtrack or story associations. Preserve the supplied audio bytes and use descriptive filenames when importing into `media/music/`.
...
Music dock controls use hover feedback only on hover-capable fine pointers, with transient pressed feedback and visible keyboard focus. Keep 44px touch targets. Use the music viewport's `viewport-fit=cover`, safe-area insets for the dock and reading frame, and minimum bottom breathing room even when insets are zero. The track selector doubles as its visible title; avoid duplicated metadata and oversized native seek bars. Test repeat modes, touch release styling, simulated safe areas, and portrait/landscape widths; simulation is not physical iPhone verification.

The site-wide Music dock is owned by `persistent-music.js`, loaded by `script.js` on every page. `music.html` remains the playable playlist source, registered by `music.js`; inactive concepts are excluded. A single top-level audio element keeps playback alive while same-origin pages load in the reading frame after listening begins. Keep normal direct URLs, independent page scripts, Back/Forward, hashes, version selectors, downloads, modified/new-tab clicks, and page notes working. Before listening, navigation remains native. Never re-create or reload audio during internal navigation. The dock includes track selection, previous/next, pause/play, seek, mute, and X to stop, release audio, and dismiss it. Reserve its height, including mobile safe areas, so content stays reachable. Media Session controls are progressive enhancements. Full reloads, external navigation, tab closure, and operating-system interruptions cannot guarantee continuous playback; do not autoplay after a reload. Verify actual repeat boundaries separately from configured loop attributes. Windows headless WebKit has paused at native loop boundaries, so iPhone/Safari repeat and background behavior need real-device verification. Run `TEST_MUSIC_ONLY=1` with the available Chromium, WebKit, and Firefox engines for focused regression coverage.
