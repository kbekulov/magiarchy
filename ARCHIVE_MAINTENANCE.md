...
Keep Gallery as one top-level destination with Artwork and Production resources collections. Existing illustrations stay Artwork unless explicitly classified otherwise. A reference can contain multiple sibling images and download formats. One production record represents one exact asset/model version, not a character's entire design history. New model revisions receive distinct stable IDs and preserved original files. `modelVersion` and `era` are author-supplied labels; never infer either. Publish only author-supplied or approved resources; private test fixtures must never become public records.
...
The female template's official name is Anima; Inanna is her unofficial nickname. Animus is the name reserved for the future male template, not an available asset. Preserve stable resource IDs and original filenames when display names change. Keep both Anima names searchable without linking the nickname to the Cult of Inanna or inferring a story connection. Future mascots may also have unofficial nicknames, displayed separately from official names.
...
Author-linked scene soundtracks retain their supplied titles and separate movements. Register a playable card for each movement with stable IDs, measured duration and file sizes, MP3 playback, and both supplied downloads. Use approved Gallery artwork as the banner, with links to the image and Moment. The Gallery card's `data-music-event` and the Moment's `soundtrack: { title, event, tracks }` open the matching Music event filter. Keep the event tag and track IDs aligned with the Music cards and individual global-search results. These are shared media associations, not revisions to scene prose or evidence that the characters hear this music. Do not infer an Arc from the recording or its filename. Import from author drop folders into `media/music/` with byte verification; preserve original filenames and audio without transcoding.
